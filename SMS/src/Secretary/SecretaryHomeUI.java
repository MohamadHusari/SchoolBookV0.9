...
public class SecretaryHomeUI extends JPanel {
	...
	public SecretaryHomeUI() {
		...
		btnAssocciate.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				Client.client.handleMessageFromClientUI(new Message("SELECT * FROM class_students;/SELECT id,name FROM users WHERE access=1;",QTypes.getassocciateclass));
				setbuttons(3);
		...
		btnExcep.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				...
				Client.client.handleMessageFromClientUI(new Message("SELECT * FROM requests WHERE aprove!='NYD';",QTypes.getrequests));
				//Disable this button and enable the rest
				setbuttons(2);
