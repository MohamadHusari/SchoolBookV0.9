...
public class AssocciateClassUI extends JPanel {
	...
	public AssocciateClassUI() {
				...
				if(tblClass.getSelectedRow()==-1){
					...
					if(tblStudent.getSelectedRow()==-1){
						...
						for(int num:tblStudent.getSelectedRows()){
							System.out.println("INSERT INTO class_students VALUES ('"+tblClass.getValueAt(tblClass.getSelectedRow(), 0).toString()+"',"+tblStudent.getValueAt(num, 0).toString()+",5);");
							lists.add("INSERT INTO class_students VALUES ('"+tblClass.getValueAt(tblClass.getSelectedRow(), 0).toString()+"',"+tblStudent.getValueAt(num, 0).toString()+",5);");
						}
						try {
							Client.client.sendToServer(new Request(lists,QTypes.insertnewstudents));
						} catch (IOException e) {
		...
		add(lblerrcl);
		lblsuc.setBounds(79, 284, 442, 14);
	...
	public void insertionsuccess(){
		lblsuc.setText("*Succesfully Added all students!Reload Page to add more!");
	...
	public void insertionfailed(){
		lblsuc.setText("*Unsuccessfuly Added all students!Reload Page to check!");
